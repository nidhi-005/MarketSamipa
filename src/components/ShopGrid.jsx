...
export default function ShopGrid({ filteredShops, query, setSelectedShop, fz }) {
  return (
    <section style={{ maxWidth: 1180, margin: "0 auto", padding: "22px 20px 60px" }}>
      ...
      {filteredShops.length === 0 ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 20 }}>
          {filteredShops.map((shop) => (
            <div key={shop.id} className="srv-card" style={{ background: "#fff", borderRadius: 16, overflow: "hidden", border: `1px solid ${TOKENS.sand}` }}>
              ...
              <div style={{ padding: "16px 18px 18px" }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={() => setSelectedShop(shop)} style={{ flex: 1, background: TOKENS.night, color: TOKENS.gold, border: "none", borderRadius: 9, padding: `${fz(10)}px 0`, fontSize: fz(13.5), fontWeight: 700, cursor: "pointer" }}>
                    View products
                  </button>
                  <a href={`tel:${shop.phone}`} title="Call shop" style={{ width: fz(42), display: "flex", alignItems: "center", justifyContent: "center", background: "#fff", border: `1.5px solid ${TOKENS.sand}`, borderRadius: 9, color: TOKENS.royal, textDecoration: "none" }}>
                    <Phone size={fz(16)} />
                  </a>
                </div>
              </div>
